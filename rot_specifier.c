...
{
    // Implementation of _print_string remains the same as before...
}
...
{
    // Implementation of _print_reversed_string remains the same as before...
}

/**
 * _print_rot13_string - Prints a ROT13'ed string.
 * @str: The string to ROT13 and print.
 * @buffer: The local buffer to store characters.
 * @index: The index to insert the character in the buffer.
 * Return: The number of characters printed.
 */
int _print_rot13_string(char *str, char *buffer, int *index)
{
    int count = 0;
    ...
    for (; *str != '\0'; str++)
    {
        if ((*str >= 'A' && *str <= 'Z') || (*str >= 'a' && *str <= 'z'))
        {
            ...
            char rotated = ((*str - base + 13) % 26) + base;
            buffer[(*index)++] = rotated;
            count++;
        }
        else
        {
...
/**
 * _printf - Custom printf function that handles %c, %s, %d, %i, %u, %o, %x, %X, %S, %p, %r, and %R specifiers.
